...
class TypedAPI:
    ...
    def get_directions(self, route: Route) -> list[Direction]:
        try:
            return [
                Direction(id=direction['id'], name=direction['name']) for direction in self.api.getdirections(route.id)['bustime-response']['directions']
            ]
        except KeyError as e:
            raise TypedAPIError('Unexpected API format')
        
    @validate_arguments
    def get_stops(self, route: Route, direction: Direction):
        try:
            return [Stop(id=stop['stpid'], name=stop['stpnm'], loc=Location(lat=stop['lat'], lon=stop['long'])) for stop in self.api.getstops(rt=route.id, direction=direction.id)['bustime-respoonse']['stops']]
        except KeyError as e:
            raise TypedAPIError('Unexpected API format.')